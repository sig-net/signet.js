...
Initiates a cross-chain transaction with response callback. The MPC network will sign the transaction, observe its execution on the target chain, and return the result.
...
| Parameter | Type | Description |
|-----------|------|-------------|
| `serialized_transaction` | `Vec<u8>` | RLP-encoded unsigned transaction for target chain |
| `caip2_id` | `String` | CAIP-2 identifier of target chain (e.g., `"eip155:1"`) |
...
Finalizes a bidirectional flow with execution results from the target chain.
