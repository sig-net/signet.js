...
const txSerialized = await chain.finalizeTransactionSigning({
...
const { hashToSign } = await chain.prepareMessageForSigning(message)
```
...
- Object containing:
  ...
  - `hashToSign`: The payload for MPC signing
...
const signature = chain.finalizeMessageSigning({
  rsvSignature,
})
```
...
- `rsvSignature`: The RSV signature from MPC
...
const { hashToSign } = await chain.prepareTypedDataForSigning(typedData)
```
...
const signature = chain.finalizeTypedDataSigning({
  rsvSignature,
})
```
